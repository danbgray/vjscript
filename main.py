...
def get_phonetic_transcription(word):
    ...
    if word in cmu_dict:
        ...
        return [map_phoneme(char) for char in word]
    ...
    for char in mapped_word:
        if char in consonant_mapping.values():
            ...
            if current_consonant:
                ...
                result += f'[v]{char}[/v]'  # Initial vowel, just add to result
        else:
...
def wrap_consonant(consonant, vowel):
    """ Wrap consonant and vowel in placeholder tags for VJScript styling """
    if vowel:
        wrapped = f'[c]{consonant}[v]{vowel}[/v][/c]'
    else:
        wrapped = f'[c]{consonant}[/c]'
    return wrapped
